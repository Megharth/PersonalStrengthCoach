...
- **Apple Health & Biometrics Sync** — Tap "Sync Biometrics" to attach Heart Rate, HRV, and Active Calories from Apple Health. When multiple workouts exist on the same day (e.g. from a wearable and a manual entry), a picker lets you choose which one to link — so the biometric samples come from the correct session instead of the service guessing.
