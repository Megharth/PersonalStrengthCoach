...
- **Apple Health & Biometrics Sync** — Tap "Sync Biometrics" to attach Heart Rate, HRV, and Active Calories from Apple Health. When multiple workouts exist on the same day (e.g. from a wearable and a manual entry), a picker lets you choose which one to link — so the biometric samples come from the correct session instead of the service guessing. Your choice is saved with the workout, so leaving the screen and coming back keeps the same workout selected instead of resetting. If that Health workout is later deleted, the picker falls back to an unselected state rather than showing a stale link.
